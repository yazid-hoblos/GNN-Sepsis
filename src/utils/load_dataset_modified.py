...
def process_dataset(dataset: dict):
    ...
    # ============================================
    # Save generated files to data/loading_output/
    # ============================================
    out_dir = f"data/loading_output/{dataset['name']}"
    import os
    os.makedirs(out_dir, exist_ok=True)

    expression_data.to_csv(f"{out_dir}/expression.csv")
    sample_info.to_csv(f"{out_dir}/sample_info.csv", index=False)
    probe_to_gene.to_csv(f"{out_dir}/probe_to_gene.csv", index=False)

    print(f"[INFO] Files saved in {out_dir}")
    print(f"  ├── expression.csv")
    print(f"  ├── sample_info.csv")
    print(f"  └── probe_to_gene.csv")
