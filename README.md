...
Fist, make sure you have all dependencies, the project was performed on Python 3.9+ with the packages listed in `requirements.txt`. You can install them using pip:
...
> [!TIP]
> make sure you get all the input files including v2.10 and v2.11 (2.9 no longer used in the project) and the paths are set correctly there  
> <details> 
> <summary>example content of `model/executions/` folder</summary>
> </details>
...
For metrics evaluation and results of protein and sample embeddings vs gene expression, the notebooks are in [metrics](./notebooks/ml/metrics.ipynb) and [results_sample](./notebooks/ml/results_sample.ipynb), and [results](./notebooks/ml/results.ipynb) respectively, metrics are saved in [`results/ml/score_tables/`](./results/ml/score_tables/) organized by seeds. Robustness analysis of performance accross seeds: [`robustness notebook`](./notebooks/ml/robustness.ipynb).

For pca/umap/tsne prrojections, run the help on this module:
```bash
python -m src.eda.project.py -h
```
